...
class Engine:
    ...
    def execute(self, command):
        ...
        if name == b"ping":
            ...
            # Unknown command. The name is binary-safe and may contain CR/LF,
            # which — echoed into an Error frame — would split the reply into
            # forged extra RESP frames (response splitting). Strip control
            # bytes and cap the length before echoing.
            cmd_name = "".join(
                chr(b) if 0x20 <= b < 0x7f else "?" for b in name[:64]
            )
    ...
    def _incr(self, args):
        ...
        if key in self.store:
            ...
            # A nil bulk stored via SET (BulkString(None)) lands here as None;
            # None.decode() would be an uncaught AttributeError -> server crash.
            # This is the same defect class as the nil command-name crash,
            # recurring in the storage path. Guard it.
            if value is None:

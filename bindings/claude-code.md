...
| Handoff and migration | A retirement followed by a spawn. The outgoing agent's handoff shard becomes the successor's brief, and the successor is launched with `agent-spawn.sh` on the new provider. The launcher's `--model` option applies to Codex only; a Claude successor starts on its configured default model, so set the tier's model before handing it the brief and record the model actually selected in the roster. See doctrine, *Quiet coordination and lifecycle*, and [doctrine/ROLES.md](../doctrine/ROLES.md). |
...
- Messages to the coordinator take fixed forms and nothing else: `LANDED #<n> <sha>`; `<worker> (<window>): BLOCKED: <one line>`; `<worker> (<window>): NEEDS RULING: <one line>`; `<worker> (<window>): COMPACT ME`; `DONE` with the remaining gaps named; the six-line standup reply; a one-line answer. The origin prefix is used only when the message does not already carry its origin.
